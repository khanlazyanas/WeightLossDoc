...
const Header = () => {
  ...
  // Handle precise scroll detection for the dynamic pill effect
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div 
        className={`fixed top-0 left-0 w-full z-[120] bg-[#0f172a] text-white overflow-hidden transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
        ...
        <div className="container mx-auto h-full px-4 sm:px-6 lg:px-8 flex justify-between items-center text-[8px] sm:text-[9px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em]">
          ...
          <div className="flex items-center gap-4 sm:gap-8">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <span className="text-slate-200">Board Certified Excellence</span>
            </div>
            <span className="hidden md:inline text-slate-600 border-l border-slate-700 pl-8">Priority Metabolic Access</span>
          </div>
          ...
          <div className="flex items-center gap-6">
            <a href="tel:+919651237672" className="hover:text-[#10b9bd] transition-colors duration-300 flex items-center gap-2 group">
              ...
              <span className="tracking-widest">+91 9651237672</span>
            </a>
          </div>
        </div>
      </div>
      ...
      <header 
        className={`fixed left-0 w-full z-[110] transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
          scrolled ? "top-4 sm:top-6" : "top-10"
        }`}
      >
        <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
          <nav 
            className={`flex items-center justify-between transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] relative ${
              scrolled 
                ? "bg-white/80 backdrop-blur-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] border border-slate-200/50 rounded-full px-4 sm:px-6 py-3 sm:py-4" 
                : "bg-transparent border-transparent rounded-none px-2 py-6 sm:py-8"
            }`}
          >
            
            {/* Logo Section */}
            <Link to="/" className="group flex items-center gap-3 sm:gap-4 relative z-[160]">
              <div className="relative">
                <div className={`flex items-center justify-center rounded-[1rem] shadow-xl transition-all duration-700 group-hover:bg-[#10b9bd] group-hover:-rotate-6 group-hover:scale-105 ${
            ...
            <div className={`hidden lg:flex items-center gap-1 absolute left-1/2 -translate-x-1/2 transition-all duration-700 ${
              scrolled ? "bg-slate-100/50 border border-slate-200/50 p-1.5 rounded-full" : ""
            }`}>
              {navLinks.map((link) => {
                ...
                return (
                  <Link 
                    key={link.name} 
                    to={link.path} 
                    className={`relative px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300 group overflow-hidden ${
            ...
            {/* Action Cluster (CTA & Hamburger) */}
            <div className="flex items-center gap-4 sm:gap-6 relative z-[160]">
              <Link 
                to="/book-appointment" 
                className={`hidden md:flex group relative items-center justify-center rounded-xl shadow-lg hover:shadow-[0_15px_30px_-10px_rgba(16,185,189,0.4)] overflow-hidden transition-all duration-300 active:scale-95 ${
              ...
              >
                ...
                <div className={`relative text-white font-black uppercase flex items-center gap-2 transition-all duration-300 ${
                  ...
                  Reserve Diagnostic
                  <svg className="w-3 h-3 transform transition-transform duration-300 group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                </div>
              </Link>
              ...
              <button 
                ...
                className={`relative w-12 h-12 lg:hidden rounded-2xl flex items-center justify-center transition-all duration-500 outline-none ${
                  isOpen 
                  ? "bg-white/10" 
                  : scrolled 
      ...
      {/* ================= REFINED MOBILE OVERLAY (FULL SCREEN MENUS) ================= */}
      <div 
        className={`fixed inset-0 z-[150] lg:hidden transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
      >
        {/* Dark Background with subtle blur/gradient */}
        <div className={`absolute inset-0 bg-[#0f172a] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
            ...
            <div className="flex flex-col gap-6 sm:gap-8">
              {navLinks.map((link, i) => (
                <Link 
                  key={link.name} 
                  to={link.path} 
                  className="group relative inline-block w-max"
                  onClick={() => setIsOpen(false)}
                >
                  <span 
                    className={`text-4xl sm:text-5xl md:text-6xl font-black text-slate-300 group-hover:text-white tracking-tighter block transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
                      isOpen ? "translate-x-0 opacity-100" : "-translate-x-12 opacity-0"
                    }`}
                    style={{ transitionDelay: `${isOpen ? i * 100 + 400 : 0}ms` }}
                  >
                    {link.name}
                  </span>
                  <div className="absolute -bottom-2 left-0 w-0 h-[3px] bg-[#10b9bd] transition-all duration-500 group-hover:w-full"></div>
                </Link>
              ))}
            </div>
          </div>

          <div className={`transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] transform mt-16 ${
            ...
            <Link 
              to="/book-appointment" 
              onClick={() => setIsOpen(false)}
              className="group flex items-center justify-between w-full bg-white text-[#0f172a] p-3 pl-8 sm:p-4 sm:pl-10 rounded-[1.5rem] hover:bg-[#10b9bd] hover:text-white transition-all duration-500 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)]"
            >
              <span className="font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-[10px] sm:text-[11px]">Start Consultation</span>
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-[#0f172a] rounded-full flex items-center justify-center text-white group-hover:scale-95 transition-transform shadow-md">
            ...
            <div className="flex justify-center gap-10 mt-12 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">
              <a href="#" className="hover:text-white hover:tracking-[0.4em] transition-all duration-300">Instagram</a>
              <a href="#" className="hover:text-white hover:tracking-[0.4em] transition-all duration-300">LinkedIn</a>
            </div>
          </div>
